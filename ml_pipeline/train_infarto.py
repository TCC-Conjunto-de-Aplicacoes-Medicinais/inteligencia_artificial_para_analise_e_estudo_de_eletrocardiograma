...
class PTBXLDataset(Dataset):
    def __init__(self, db_path, folds=[1,2,3,4,5,6,7,8], task='infarto', sampling_rate=100):
        ...
        for idx, row in self.df.iterrows():
            ...
            # Agente 1 (Infarto) foca em Myocardial Infarction (MI)
            label = 1 if 'MI' in superclasses else 0
...
def train_agent_infarto(epochs=5, batch_size=64):
    print("🚀 Iniciando treinamento do Agente 1 - Infarto Agudo do Miocárdio (IAM)...")
    ...
    if os.path.exists(db_path):
        ...
        train_dataset = PTBXLDataset(db_path, folds=list(range(1, 9)), task='infarto', sampling_rate=100)
        val_dataset = PTBXLDataset(db_path, folds=[9, 10], task='infarto', sampling_rate=100)
    ...
    with torch.no_grad():
        for batch_x, batch_y in val_loader:
            batch_x = batch_x.to(device)
            outputs = model(batch_x)
            # A probabilidade da classe positiva (Infarto = 1)
    ...
    plt.title('Curva ROC - Agente Infarto (IAM)')
    plt.legend(loc="lower right")
    
    # Salvar em docs/auc_roc_infarto.png
    docs_dir = os.path.join(os.path.dirname(__file__), "..", "docs")
    os.makedirs(docs_dir, exist_ok=True)
    plot_path = os.path.join(docs_dir, "auc_roc_infarto.png")
    plt.savefig(plot_path)
    plt.close()
    print(f"✅ Gráfico salvo com sucesso em: {plot_path}")

    output_path = os.path.join(os.path.dirname(__file__), "..", "backend_go", "models_onnx", "agente_infarto.onnx")
